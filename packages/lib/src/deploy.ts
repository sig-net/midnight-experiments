...
import { submitUnprovenTransaction, type AccountKeys, type TransactionIdentifier } from "./wallet.ts";
...
/**
 * Deploy a compiled contract through an ALREADY-OPEN wallet facade — the
 * benchmark sessions open one synced facade per test file and reuse it for
 * every deploy + call, instead of paying the facade start/sync cost per
 * deploy the way the standalone deploy scripts do.
 *
 * @param facade - A started (and synced) wallet facade that pays for and submits the deploy.
 * @param keys - The key material of the same wallet.
 * @param networkId - The network the transaction targets.
 * @param compiledContract - The bound contract, from {@link makeVacantCompiledContract}.
 ...
 */
export async function deployWithFacade<C extends Contract.Contract<PS>, PS>(
  facade: WalletFacade,
  keys: AccountKeys,
  networkId: NetworkId,
  compiledContract: CompiledContract.CompiledContract<C, PS>,
  initialPrivateState: PS,
  ...constructorArgs: Contract.Contract.InitializeParameters<C>
): Promise<{ contractAddress: string; txId: TransactionIdentifier }> {
  ...
  return { contractAddress: deployTransaction.contractAddress, txId };
}
