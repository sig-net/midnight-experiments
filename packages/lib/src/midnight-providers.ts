...
export function createCrossContractProofServerProvider(
  ...
  return createProofProvider(buildProvingProvider(proofServerUrl, zkConfigProviders));
}

/**
 * The circuit-level {@link ProvingProvider} behind
 * {@link createCrossContractProofServerProvider}, exposed separately so the
 * benchmark instrumentation (bench.ts) can wrap its `check`/`prove` with
 * timing + size recording before lifting it into a {@link ProofProvider}.
 *
 * @param proofServerUrl - The proof server's HTTP endpoint.
 * @param zkConfigProviders - One provider per compiled contract in the call tree; must be non-empty.
 * @returns The proving provider (check/prove/lookupKey) resolving keys across all given contracts.
 * @throws If `zkConfigProviders` is empty.
 */
export function buildProvingProvider(
  proofServerUrl: string,
  zkConfigProviders: readonly ZKConfigProvider<string>[],
): ProvingProvider {
  if (zkConfigProviders.length === 0) {
    throw new Error("buildProvingProvider: at least one zkConfigProvider is required");
  }
